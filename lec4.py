...
@app.post('/users')
async def get_users(user: User):  # user is of BaseModel class so FastAPI knows it is a request object
    print(user.name, user.age)    # instead of request.name, we now have to use the class for receiving and using the request
    return user
